...
- **Dual-maintenance window** — store/client bugfixes must be applied to both old `owservable` and new core until consumers migrate. Keep the window short (migrate systools-server first, as the primary consumer), then freeze `owservable` to critical fixes — or convert it into a metapackage that re-exports `@owservable/core` + `@owservable/mongodb` for drop-in compatibility.
